...
hdf5_path = 'ilsvrc_train.hdf5'
img_path = '/data1/nithish/WORK/Resources/ilsvrc_train/*.png'
addrs = glob.glob(img_path)

train_addrs = addrs[0:int(1.0*len(addrs))]
train_list = open('new_train.txt').readlines()
for i in xrange(len(train_list)):
   train_list[i] = train_list[i].split()[0]
print "{} {}".format("Length of train list",len(train_list))
shuffle(train_list)
...
for i in range(len(train_list)):
    ...
    #addr = train_addrs[i]
    os.chdir(img_dir)   
    img = imread(train_list[i])
    os.chdir(curr_dir)
    img = resize(img, (224, 224))*255.0
    img2 = img
    img = img.reshape((1,size,size,3))
 
    hdf5_file["train_img"][i, ...] = img2
